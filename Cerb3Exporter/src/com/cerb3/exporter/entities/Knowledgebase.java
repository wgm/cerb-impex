...
public class Knowledgebase { 	
	...
	public void export() {
		...
		try {
			...
			stmtArticles.execute("SELECT kb.id, kb.title, kb.views, kbc.content "+
					"FROM `kb` "+
			...
			while(rsArticles.next()) {
				...
				stmtKbCategories.execute(String.format("SELECT kbc.kb_id, kbc.kb_category_id "+
						"FROM `kb_to_category` kbc "+
						"WHERE kbc.kb_id = %d",
						iId
					));
